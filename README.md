...
## Geração dos Gráficos e Exportação do PDF

Requer Python 3.10 ou superior para executar os scripts de apoio desenvolvidos pelo grupo:
...
- `gerar_graficos.py`: processa a tabela de medianas (`dados/resultados.csv`) e plota as curvas comparativas em PNG salvas na pasta `relatorio/`.
- `gerar_pdf.py`: utilitário para converter e diagramar o `RELATORIO.md` em formato PDF pronto para submissão no AVA.

Os arquivos de dados volumosos são ignorados pelo Git para manter o repositório enxuto. Os códigos-fonte Java, scripts de benchmark, gráficos e o relatório final ficam devidamente versionados.

Para usar um Java fora do PATH nos scripts Python, defina a variável `JAVA` com o caminho completo do executável. Isso não muda o comando de compilação.
